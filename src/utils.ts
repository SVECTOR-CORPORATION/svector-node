...
export async function toFile(
  ...
  if (value instanceof Uint8Array) {
    // Create a new Uint8Array with ArrayBuffer to ensure BlobPart compatibility
    const safeUint8Array = new Uint8Array(value);
    const blob = new Blob([safeUint8Array], { type: options?.type || 'application/octet-stream' });
    return new File([blob], filename || 'file', { type: blob.type });
  }
